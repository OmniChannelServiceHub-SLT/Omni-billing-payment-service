// src/APIs/createEBillDownloadRequest/controllers/eBillDownloadRequestController.js
...
const service = require(
  '../services/eBillDownloadRequestService'
);

const mapper = require(
  '../mappers/eBillDownloadRequestMapper'
);
 ...
 */
async function createEBillDownloadRequest(
  req,
  res
) {
  try {
    ...
    const record =
      await service.getEBillDownloadRequest(
        accountNo,
        ebillMonth,
        tpNo
      );
    ...
    if (!record) {
      return failure(res, {
        message:
          `No E-Bill download record found for accountNo '${accountNo}' and month '${ebillMonth}'.`,
        errorCode: 'E404',
        status: 404,
      });
    }
    ...
    console.error(
      '[EBillDownloadRequest] Failed:',
      error.message
    );

    return failure(res, {
      message:
        'Failed to retrieve the E-Bill download request.',
      errorCode: 'E500',
      exceptionDetail:
